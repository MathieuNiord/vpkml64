...
sig
  ...
  val get_commands_names : t -> string list (* Returns the names of the options in the menu *)
...
struct
  ...
  let get_commands_names (m : t) : string list =
    let commands = get_commands m in
    List.map (Cmd.get_name) commands ;;
...
let show (menu : t) : unit =
  ...
  let rec aux (choice : int) : unit =
    let display, cols = build_options (get_commands_names menu) choice in (
      ...
      let input : Input.t = Input.get () in (
        let new_choice : int = Input.process input (List.length options) choice cols in (* Computing the new choice *)
          if (input = ENTER) then run_command menu choice (* Running the selected option *)
